...
exports.pathToBareSpecifiersTransform = (assetsDirectory) => stream_js_1.transformStream((file) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        log.info(`Applying reverse empathy to ${relativePath}`);
        const specifiers = [];
        ...
        for (const specifier of specifiers) {
            ...
            try {
                ...
                log.info(`Adjusting specifier '${originalSpecifier}'
  New value: ${specifier.value}`);
            }
            catch (error) {
                log.info(`Failed to adjust specifier '${originalSpecifier}'`);
                log.error(error);
            }
        }
        ...
        log.error(`Failed to transform specifiers in ${relativePath}`);
        log.error(error);
    }
    return file;
}));

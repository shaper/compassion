...
// NOTE(shaper): This was copied from https://github.com/PolymerLabs/html-modules-toolkit.
// It was originally symlinked via html-hd, but that repo is gone now.
...
export type DestructureFunction<T> = (structured: T, encoding?: string) =>
    Promise<T[]>

    export const destructureStream = <T>(
        fn: DestructureFunction<T>, options?: StreamOptions): Transform =>
        asyncThrough(
...
export type TransformFunction<T, U = T> = (source: T, encoding?: string) =>
    Promise<U>;

export const transformStream =
    <T, U = T>(fn: TransformFunction<T, U>, options?: StreamOptions):
        Transform => asyncThrough(
            async(stream: Transform, chunk: T, encoding?): Promise<void> => {
